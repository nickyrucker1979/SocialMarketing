...
exaconnect.execute('delete from UNIVERSITY_WAREHOUSE_DEV.GA_PAGE_TRACKING')
...
results = service.data().ga().get(
    ...
    end_date='today',
    dimensions='ga:pagePathLevel1, ga:pagePathLevel2, ga:pagePathLevel3, ga:pagePathLevel4, ga:pageTitle, ga:landingPagePath, ga:secondPagePath', # ga:secondPagePath', #
    metrics='ga:pageviews, ga:uniquePageviews, ga:timeOnPage, ga:exits ',
...
while start_index < totalResults:
    results = service.data().ga().get(
          ...
          start_date='2017-01-01',
          end_date='today',
          dimensions='ga:pagePathLevel1, ga:pagePathLevel2, ga:pagePathLevel3, ga:pagePathLevel4, ga:pageTitle, ga:landingPagePath, ga:secondPagePath',
          metrics='ga:pageviews, ga:uniquePageviews, ga:timeOnPage, ga:exits',
          samplingLevel='HIGHER_PRECISION',
    ...
    for row in getValue(results, ['rows']):
      exa_write = pd.DataFrame({
            ...
            'f': (getValue(row, [5])).replace('(not set)', ''),
            'g': (getValue(row, [6])).replace('(not set)', ''),
            'h': getValue(row, [7]),
            'i': getValue(row, [8]),
            'j': getValue(row, [9]),
            'k': getValue(row, [10]),
      }, index=[0])
      exaconnect.writePandas(exa_write, 'UNIVERSITY_WAREHOUSE_DEV.GA_PAGE_TRACKING')
