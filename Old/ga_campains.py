...
exaconnect.execute('delete from UNIVERSITY_WAREHOUSE_DEV.GA_CAMPAIGNS')
...
results = service.data().ga().get(
    ...
    end_date='today',
    dimensions='ga:campaign, ga:source, ga:adGroup, ga:date, ga:adwordsCampaignID',
    metrics='ga:impressions,ga:adClicks,ga:adCost,ga:CPM,ga:CPC,ga:CTR,ga:costPerTransaction,ga:costPerGoalConversion,ga:costPerConversion',
...
while start_index < totalResults:
    results = service.data().ga().get(
          ...
          start_date='2017-01-01',
          end_date='today',
          dimensions='ga:campaign, ga:source, ga:adGroup, ga:date',
          metrics='ga:impressions,ga:adClicks,ga:adCost,ga:CPM,ga:CPC,ga:CTR,ga:costPerTransaction,ga:costPerGoalConversion,ga:costPerConversion',
          samplingLevel='HIGHER_PRECISION',
    ...
    for row in getValue(results, ['rows']):
      exa_write = pd.DataFrame({
             'a': (removePipe(getValue(row, [0]))).replace('(not set)',''),
            'b': (getValue(row, [1])).replace('(direct)',''),
            'c': (getValue(row, [2])).replace('(not set)',''),
            'd': converttime(getValue(row, [3])),
            'e': getValue(row, [4]),
            'f': getValue(row, [5]),
            'g': getValue(row, [6]),
            'h': getValue(row, [7]),
      ...
      # exaconnect.writePandas(exa_write, 'UNIVERSITY_WAREHOUSE_DEV.GA_CAMPAIGNS')
      print(exa_write)
